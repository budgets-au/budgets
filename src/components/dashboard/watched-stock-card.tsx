 ...
 *  Mirrors `TrackedStockCard` shape — same 2×2 footprint, the same
 *  in-edit dropdown selector + symbol/price/sparkline render — but
 *  pulls from `/api/watchlist` instead of `/api/investments`. The
 *  active selection lives in the layout entry's
 *  `config.watchlistId`. Day-change is derived from the last two
 *  points of the history series (the watchlist list endpoint
 *  doesn't carry a prior-close column, unlike investments). */
export function WatchedStockCard({
  config,
  editMode,
  onConfigChange,
}: {
  ...
  const watchlistId =
    typeof config?.watchlistId === "string" ? config.watchlistId : null;

  const { data: watchlistData } = useSWR<WatchlistRow[]>(
    "/api/watchlist",
    fetcher,
    { revalidateOnFocus: false },
  );
  // Defensive cast — if /api/watchlist returns an error-shaped body
  // (e.g. unauth), fall back to an empty list rather than crashing
  // on .find().
  const watchlist: WatchlistRow[] = Array.isArray(watchlistData)
    ? watchlistData
    : [];

  const selected = watchlistId
    ? watchlist.find((w) => w.id === watchlistId) ?? null
    : null;

  const { data: historyData } = useSWR<HistoryResp>(
    selected ? `/api/watchlist/${selected.id}/history?range=1m` : null,
    fetcher,
    { revalidateOnFocus: false },
  );
  const history = historyData?.series ?? [];

  // Derive day-change from the tail of the history series — the
  // list endpoint doesn't surface a prior-close column.
  const lastClose = history.length > 0 ? history[history.length - 1].close : null;
  const priorClose =
    history.length > 1 ? history[history.length - 2].close : null;
  const change =
    lastClose != null && priorClose != null ? lastClose - priorClose : null;
  const changePct =
    change != null && priorClose != null && priorClose !== 0
    ...
    <Card data-size="sm" className="h-full flex flex-col">
      <CardHeader className="pb-1 shrink-0 flex flex-row items-center justify-between space-y-0">
        <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-1.5">
          ...
          {selected ? (
            <Link
              href={`/investments?tab=watchlist&id=${selected.id}`}
      ...
      <CardContent className="flex-1 min-h-0 flex flex-col">
        {editMode && (
          <select
            value={watchlistId ?? ""}
            onChange={(e) =>
              onConfigChange?.({ watchlistId: e.target.value || null })
            }
            className={cn(
            ...
            aria-label="Pick a watched stock"
          >
            <option value="">— Pick a watched stock —</option>
            {watchlist.length === 0 ? (
        ...
        {!selected ? (
          <p className="text-xs text-muted-foreground">
            {editMode
              ? "Pick a watched stock from the dropdown."
          ...
          <>
            <p className="text-2xl font-bold">
              {selected.currentPrice != null
                ...
                : lastClose != null
                  ? `${selected.currency === "USD" ? "US$" : "A$"}${lastClose.toFixed(2)}`
                  : "—"}
            </p>
            ...
            <div className="flex-1 min-h-0 mt-2 -mx-1">
              {editMode ? (
                ...
                // update-depth limit. Matches TrackedStockCard.
